...
// Objects
/**
 * we create our objects geometry here
 */
...
// I added a texture by loading the desired image into the geometry where it is added
const textureCube = new THREE.TextureLoader().load('blue_wool.jpg')
...
// the objects are added to the scene 
scene.add(torusKnot)
scene.add(cube)
scene.add(cylinder)
scene.add(torus)
...
const animate = () => {
  ...
  // Update objects
  // this objects are updated every time the function calls itself.
  // the rotation of the objects goes about the z axes.
  ...
  torus.scale.set(.1 * elapsedTime,.1 * elapsedTime,.1 * elapsedTime)
